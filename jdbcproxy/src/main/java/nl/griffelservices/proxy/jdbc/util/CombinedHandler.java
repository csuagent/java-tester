...
/**
 * This handler combines the functionality of the {@link TracerHandler},
 * the {@link StubTracerHandler} and the {@link StubHandler}.
 * The handler itself implements the methods of the <code>java.sql.Driver</code> class
 * and for the other JDBC interfaces it passes control to one of the three other handlers.
 ...
 *     <ul>
 *     <li><code>[filename]</code> is the name of the file to which the trace information must be written.
 *         If this element is empty, then the trace information is logged to <code>System.out</code>.
 *     <li><code>[driver]</code> is the name of the actual JDBC driver to which the calls will be passed.
 *     <li><code>[driverURL]</code> is the URL that must be passed to the actual JDBC driver.
 *     </ul>
 * <li><code>jdbc:stubtracer:[foldername]:[driver]:[driverURL]</code>
 *     <ul>
 *     <li><code>[foldername]</code> is the name of the folder to which the stub information must be written.
 *         If this element is empty, then the stub information is logged to <code>System.out</code>.
 *     <li><code>[driver]</code> is the name of the actual JDBC driver to which the calls will be passed.
 *     <li><code>[driverURL]</code> is the URL that must be passed to the actual JDBC driver.
 *     </ul>
 * <li><code>jdbc:stub:[host]:[port]:[timeout]:[driverstubdata]</code>
 *     <ul>
 *     <li><code>[host]</code> is the hostname of the HTTP server that provides the stub information.
 *     <li><code>[port]</code> is the port of the HTTP server that provides the stub information.
 *     <li><code>[timeout]</code> is the timeout that will be used for HTTP requests to the HTTP server that provides the stub information.
 *     <li><code>[driverstubdata]</code> is the optional filename of a response file that can provide the proxy data for the stub driver.
 *         If this field is empty (or not present), then <code>new ProxyObject(Driver.class, "0", "0", null)</code>
 *         is used as the proxy data for the stub driver.
 *         If this field is not empty, then the <code>returnvalue</code> in the response file, which must be a {@link ProxyObject},
 *         will be used for the stub driver. The <code>newstatus</code> in the response file will be ignored.
 *     </ul>
 * </ul>
 * Note that the <code>:</code> character is used to separate the parts in the driver URL.
 * This implies that the individual parts in the driver URL, except for the last part, should not contain that character.
 * 
...
public class CombinedHandler implements Handler
{
  ...
  private static final Method connect = Proxy.getMethod(Driver.class, "connect", new Class[] { String.class, Properties.class });
  ...
  private static final Method getPropertyInfo = Proxy.getMethod(Driver.class, "getPropertyInfo", new Class[] { String.class, Properties.class });
      ...
      if (parts.length == 3 && parts[0].equals(JDBC))
      {
        ...
        if (parts[1].equals(STUB))
        {
          ...
          if (parts.length >= 3)
          {
            ...
            {
              proxyObject = (ProxyObject)responseDecoder.decode(readFile(new File(parts[3]))).getReturnValue();
            }
            else
            {
