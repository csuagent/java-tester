...
public class StubTracerMerger {
	 ...
	 * response files are located. The second argument is the optional proxy id
	 * of the root response (default is 0). The third argument is the optional
	 * proxy status of the root response (default is 0).
	 * 
	 * @param args
	...
	/**
	 * Constructs a StubTracerMerger object. Loads all (decoded) request and
	 * response files, but does not merge them yet.
	 * 
	 * @param folder
	 *            the folder in which the request and response files are located
	 * @throws Exception
	 *             if an error occurs
	 */
		...
		if (returnValue instanceof ProxyObject) {
			...
			while (it.hasNext()) {
				...
				if (po.getProxyId().equals(rr.getRequest().getDesiredId())) {
					Response response = new Response(rr.getResponse().getNewStatus(), expandReturnValue(rr
