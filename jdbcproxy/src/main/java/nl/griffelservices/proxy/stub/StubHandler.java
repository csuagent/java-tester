...
/**
 * This handler is meant to provide stub responses for the calls to the proxied interfaces.
 * The actual stub responses are provided by a {@link Stub} class.
 * This driver uses the {@link ProxyObject} as the proxy data.
 * Note that this proxy data already contains pre-configured stub responses for a set of requests.
 * This means that for these request, the {@link Stub} does not need to be invoked.
 * 
...
public class StubHandler implements Handler
{
   ...
   * 
   * @param stub the object that provides the actual stub responses
   */
  ...
  public Object invoke(Proxy proxy, Method method, Object[] parameters) throws Exception
  {
    ...
    {
      throw new IllegalArgumentException("Number of parameters " + parameters.length + " does not match method " + method);
    }
    ProxyObject proxyObject = (ProxyObject)proxy.getProxyObject();
    
    // first see whether a response for this call has already been loaded in the proxy object
    for (int i = 0; i < proxyObject.getRequestResponseCount(); i++)
    {
      if (proxyObject.getRequestResponse(i).getRequest().matches(proxyObject.getProxyId(), proxyObject.getProxyStatus(), method, parameters))
      {
    ...
    Request request = new Request(proxyObject.getProxyId(),
                                  new Request.EqualityParameter(proxyObject.getProxyStatus()),
                                  method,
                                  equalityParameters);
