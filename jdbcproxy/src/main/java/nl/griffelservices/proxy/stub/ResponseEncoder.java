...
public class ResponseEncoder extends Encoder {
	...
	private Element encodeObject(Document document, Object object, String elementName) {
		...
		if (objectType == null) {
			...
			for (int i = 0; i < proxyObject.getRequestResponseCount(); i++) {
				...
				requestResponseElement.appendChild(encodeRequest(document, proxyObject.getRequestResponse(i)
						.getRequest()));
				requestResponseElement.appendChild(encodeResponse(document, proxyObject.getRequestResponse(i)
						.getResponse()));
	 ...
	 * Note that the encoding done here is not the same as in
	 * {@link RequestEncoder#encode(Request)}. Since the encoding done here is
	 * intended to be enclosed in the encoding of a {@link ProxyObject}, there
	 * is no need to encode the class and id of the request.
	 * 
	 * @param document
	 *            the document for which the element is meant
	 * @param request
