...
/**
 * Generates the proxy classes, and their common super class, for a given set of interfaces.
 * 
...
public class Generator
{
   ...
   * 
   * @param interfaces the interfaces for which proxy classes must be generated
   * @param proxyPackage the name of the package of the proxy classes
   * @param superClassName the name of the common super class of the proxy classes
   */
  ...
  /**
   * Generates the proxy classes, and their common super class, files in the specified folder.
   * 
   * @param folder
  ...
  /**
   * Generates the base class that implements the generic functionality for all proxt classes.
   * 
   * @param pw the writer to which the generated code must be written
   */
  private void generateSuperClass(PrintWriter pw)
  {
    ...
    for (int i = 0; i < interfaces.length; i++)
    {
      ...
      pw.println("    if (returnType.equals(" + fullClassName + ".class)) return new " + className + "Proxy(handler, proxyObject);");
    }
   ...
   * 
   * @param pw the writer to which the generated code must be written
   * @param interfaceIndex the index in the {@link #interfaces} array
   */
  ...
  /**
   * Generates the comment that indicates it is a generated class.
   * This comment is used both in teh proxy classes and in the common super class.
   * 
   * @param pw the writer to which the generated code must be written
   */
   ...
   * 
   * @param pw the writer to which the generated code must be written
   * @param fullClassName the name of the interface whose method implementation is generated 
   * @param count the method index (used to name method specific static variables)
   * @param method the method whose implementation must be generated
   */
  private static void generateMethod(PrintWriter pw, String fullClassName, int count, Method method)
  {
    ...
    for (int i = 0; i < arg.length; i++)
    {
      ...
      if (i > 0) argDeclaration += ", ";
      argDeclaration += name(arg[i]) + " p" + i;
      
      if (i > 0) argValues += ", ";
      if (arg[i].equals(boolean.class)) argValues += "new Boolean(p" + i + ")";
      else if (arg[i].equals(byte.class)) argValues += "new Byte(p" + i + ")";
      else if (arg[i].equals(char.class)) argValues += "new Character(p" + i + ")";
      else if (arg[i].equals(double.class)) argValues += "new Double(p" + i + ")";
      else if (arg[i].equals(float.class)) argValues += "new Float(p" + i + ")";
      else if (arg[i].equals(int.class)) argValues += "new Integer(p" + i + ")";
      else if (arg[i].equals(long.class)) argValues += "new Long(p" + i + ")";
      else if (arg[i].equals(short.class)) argValues += "new Short(p" + i + ")";
      else argValues += "p" + i;
    }
    ...
    pw.println();
    pw.println("  /** " + method + " */");
    pw.println("  private static final Method m" + count + " = getMethod(" + fullClassName + ".class, \"" + method.getName() + "\", new Class[] {" + argClasses + "});");
    pw.println("  public " + name(ret) + " " + method.getName() + "(" + argDeclaration + ")" + excThrows);
    pw.println("  {");
    String invoke = "invoke(m" + count + ", new Object[] {" + argValues + "})";
    if (ret.equals(boolean.class)) invoke = "return ((Boolean)" + invoke + ").booleanValue()";
    else if (ret.equals(byte.class)) invoke = "return ((Byte)" + invoke + ").byteValue()";
    else if (ret.equals(char.class)) invoke = "return ((Character)" + invoke + ").charValue()";
    else if (ret.equals(double.class)) invoke = "return ((Double)" + invoke + ").doubleValue()";
    else if (ret.equals(float.class)) invoke = "return ((Float)" + invoke + ").floatValue()";
    else if (ret.equals(int.class)) invoke = "return ((Integer)" + invoke + ").intValue()";
    else if (ret.equals(long.class)) invoke = "return ((Long)" + invoke + ").longValue()";
    else if (ret.equals(short.class)) invoke = "return ((Short)" + invoke + ").shortValue()";
    else if (ret.equals(void.class)) invoke = invoke + "";
    else invoke = "return (" + name(ret) + ")" + invoke;
    pw.println("    " + invoke + ";");
    pw.println("  }");
  }
   ...
   * 
   * @param c the class for which teh name is needed
  ...
  private static String name(Class c)
  {
    ...
    if (name.charAt(0) == '[')
    {
      ...
      {
        case 'Z': return "boolean" + arrayName;  
        case 'B': return "byte" + arrayName;  
        case 'C': return "char" + arrayName;  
        case 'D': return "double" + arrayName;  
        case 'F': return "float" + arrayName;  
        case 'I': return "int" + arrayName;  
        case 'J': return "long" + arrayName;  
        case 'S': return "short" + arrayName;  
        case 'L': return name.substring(1, name.length() - 1) + arrayName;  
