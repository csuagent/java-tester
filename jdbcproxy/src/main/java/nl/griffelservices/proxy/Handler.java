 ...
 * makes it possible for a single handler class to implement many different
 * interfaces in a consistent way. Handlers can pass the calls to another
 * implementation of the interfaces; adding extra functionality like tracing or
 * access control. Handlers can also provide an implementation of the interfaces
 * that is completely different from other implementations.
 * 
 * @author Frans van Gool
 */
public interface Handler {
	/**
	 * Called when a proxy class is constructed. This is just a notification to
	 * the handler; the handler is allowed to ignore this call.
	 * 
	 * @param proxyClass
	 *            the proxy class
	 * @param proxyObject
	 *            the data passed to the proxy class constructor
	 */
	public void init(Class<?> proxyClass, Object proxyObject);

	/**
	 * Called when an interface method of a proxy class is called. This is the
	 * most crucial method of a handler; it must return the value that is
	 * expected from the interface method call.
	 * 
	 * @param proxy
