...
public class EsbChannelTaskManagerFixture extends DtoPropertyFixture {
	...
	public void createChannelTask(ChannelTaskDto dto) {
		...
		// want.bool(result.isSuccess()).isEqualTo("返回结果必须是" + dto.esbResult,
		// dto.esbResult);
	}
	...
	public static class ChannelTaskDto {
		...
		public HashMap<String, Object> getChannelTaskMap() {
			...
			channelTask.put(ChannelTaskConstant.SEND_TYPE,
					ChannelConstant.EMAIL);
			// test case value
			...
			channelTask.put(ChannelTaskConstant.CUSTOMER_FATIGUE_RULES,
					new String[] { customerFatigueRules });
			channelTask
					.put(ChannelTaskConstant.FATIGUE_ENABLED, fatigueEnabled);
			channelTask.put(ChannelTaskConstant.BLACKLIST_NAMES, StringHelper
					.isBlankOrNull(blacklistNames) ? null
			...
			channelTask
					.put(ChannelTaskConstant.CHANNEL_SETTING, channelSetting);
		...
		public String toString() {
			...
			// json
			buff.append("\n").append("channelTask:").append(
					JSONObject.fromObject(getChannelTaskMap()).toString());
